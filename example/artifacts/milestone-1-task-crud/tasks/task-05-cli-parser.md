...
- **Outcome**: Parser and entrypoint implemented; `runMigrations` called unconditionally before dispatch. 8 tests added covering `--help`, no-args, unknown-command, and `--all` routing. Final pass, so the full suite ran.
