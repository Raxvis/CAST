...
loop defined in docs/PIPELINE_LOOP.md (Coder → Tester → Reviewer → validation, with
...
- [product](../../agents/product.md) — triages bug reports, validates tasks Reviewer's criteria check flagged (Step 4b), disposes of task-amendment proposals, re-triages Deferred bugs and tasks at milestone completion, and writes the milestone completion and validation records (verifying the CEO Approval Conditions as part of them)
...
- [docs-writer](../../agents/docs-writer.md) — drains the `docs` queue from `artifacts/STANDUP.md` at the milestone-completion checkpoint, plus an overflow drain if the queue passes its bound mid-milestone
- [validator](../../agents/validator.md) — records task and milestone outcomes in `artifacts/AGENT_STATE.md` in one milestone-completion pass, writes the milestone retrospective, and (invoked mid-loop) pauses the test gate when Tester flags an Environment Issue
...
- **Reviewer (Step 3)** appends the **Acceptance Criteria Check** to its approval entry — one line per acceptance criterion, each `Met` with an evidence pointer, `Not met`, or `Product judgment`. This block is what decides whether Step 4 spawns Product.
- **Validation (Step 4)** is conditional. A task whose criteria Reviewer marked all `Met` closes via **Step 4a** — the orchestrator writes the Status and the `progress` entry, no Product launch. Any `Not met` or `Product judgment` criterion, any mid-task criterion amendment, or any task carrying a CEO Approval Condition routes to **Step 4b** and launches **product** as before. Product validates against the task file's acceptance criteria, applying the Task Validation Checklist in `templates/MILESTONE_VALIDATION.md` as the *criteria*. Either way the outcome is recorded as the task file's Status (Header) plus a `progress` entry in `artifacts/STANDUP.md` — no per-task validation document is produced; the validation *document* is written once, at the milestone-completion checkpoint, and it covers every task including those closed via 4a.
...
4. **Checkpoints are serialized.** Task-completion checkpoints (and the milestone-completion checkpoint) run one at a time — they touch the shared root files by design. This holds even though the task-completion checkpoint now launches no agents: its Status writeback, `progress` entry, and any overflow drain are still shared-root writes, and the overflow-drain count must be taken and acted on without another task interleaving a `docs` entry.
...
Run this checkpoint each time a task passes validation (`docs/PIPELINE_LOOP.md` Step 4) — including single-task invocations. **It launches no agents**: the bookkeeping roles are batched to the milestone-completion checkpoint below, so a clean task costs the loop's own stages and nothing more.

1. **Status writeback.** Mark the task's **Status** as Complete in its own file's Header — the single place task status lives (the README's Task Index deliberately has no status column). This writeback is what makes the skill resumable (see Task Selection); it is a file write by the orchestrator, not an agent launch.
2. **Progress entry.** Append the task's `progress` entry to `artifacts/STANDUP.md` under this run's session heading, per Step 4a or 4b of the loop doc.
3. **Overflow drain (conditional).** Count the pending `docs` entries in `artifacts/STANDUP.md` — the `- <agent> | docs | <note>` lines not yet marked ✅. If the count is **10 or more**, invoke the **docs-writer** agent now to drain them, then continue. This bound keeps a long milestone's queue from growing unbounded without paying a drain per task; below the bound, the queue waits for the milestone-completion drain. Nothing is lost either way — the queue is append-and-drain by design and every entry carries its own context.

**Why these are batched.** Docs Writer and Validator produce records, not code, and both consume a queue or a state file rather than the task's live context — running them once per task spent two cold subagent contexts per task on bookkeeping that reads identically when batched. Resumability does not depend on them: the task file's Status field (step 1) is what Task Selection reads on a resumed run, and `artifacts/AGENT_STATE.md` is a dashboard, not the resume record.
...
7. **Docs Writer.** Invoke the **docs-writer** agent to drain every pending `docs` entry from `artifacts/STANDUP.md` (per its Entry Grammar; drained entries are marked ✅). This is the milestone's primary drain — with the task-completion checkpoint batched, the queue reaching here holds every entry the milestone produced that no overflow drain already took.
8. **Validator.** Invoke the **validator** agent to record **every task outcome in this milestone** plus the milestone outcome in `artifacts/AGENT_STATE.md` (Agent Status Dashboard and Milestone Progress tables) — one invocation covering all tasks, reading each task file's Header and its Handoff Log, rather than one invocation per task. In the same pass Validator runs its per-task Process Checklist across the milestone's tasks. Then write the milestone retrospective at `artifacts/milestone-{N}-{slug}/reviews/retrospective.md` using `templates/MILESTONE_RETROSPECTIVE.md`. As part of the same invocation, Validator performs archive hygiene (its Archival Duty section defines the mechanics): every `artifacts/one-off/task-{slug}.md` with Status Complete moves to `artifacts/one-off/archive/` (bug files stay put — the index rows point at them), closed session sections in `artifacts/STANDUP.md` and stale rows in `artifacts/AGENT_STATE.md` move to `artifacts/archive/`, keeping both live files bounded to the current milestone plus a tail.
