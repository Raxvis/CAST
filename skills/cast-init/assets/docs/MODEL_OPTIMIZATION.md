...
Every agent's YAML frontmatter sets `model: inherit` — each agent runs on the model of the invoking session, with Claude Opus 5 as the preferred executing model (Opus 4.8 / 4.7 / 4.6 supported). The per-role **effort** column below is *advisory guidance carried in each agent's prompt body*, not an enforced setting: Claude Code subagent frontmatter has no effort key, so under `inherit` every agent runs at the session's effort and the stated default tells the agent how to spend its reasoning, not the platform what to allocate. The one mechanically enforceable per-role lever is the frontmatter `model:` pin (see Right-sizing below). (The pre-v0.11.0 template used Opus 4.6 / Sonnet 4.6 / Haiku 4.5 tiers; see Upgrade Paths below if you are migrating an older install.)
