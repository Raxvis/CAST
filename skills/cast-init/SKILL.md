...
2. **All 8 agents exist** with frontmatter matching the canonical roles (a `ui` absence on a backend/CLI-only project requires a recorded opt-out; every other absence is a hard failure). No v2 agent file survives in `.claude/agents/`.
