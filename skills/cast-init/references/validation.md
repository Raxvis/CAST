...
7. **Verify topic-doc pairing and imports**:
   ...
   - Each imported path must exist in the target project. A dangling import is an error. The block itself is expected to carry **no** import lines on a fresh install — the list ships empty by design (execution step 5.8.4), so a topic doc absent from the imports is correct, not a gap. On a merge, only lines the user already had should appear; an import CAST added is the failure to flag.
