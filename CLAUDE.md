...
- **`skills/cast-init/assets/`** — The installable payload. Everything below is relative to this directory:
  ...
  - **`skills/`** — Pipeline skill definitions (`/agent-plan`, `/agent-code`, `/agent-task`) that orchestrate the planning and engineering stages of the agent workflow plus a mini pipeline for one-off tasks, and the `/cast-doctor` maintenance skill (run-anytime install health check and model-aware documentation audit). Installed to `.claude/skills/` in the target project. Each SKILL.md's frontmatter `name` must equal its directory name
