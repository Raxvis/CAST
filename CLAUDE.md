...
- **`skills/cast-init/assets/`** — The installable payload. Everything below is relative to this directory:
  ...
  - **`agents/`** — 7 agent role definitions plus a master `README.md` (product, architect, ui, ceo, coder, reviewer, docs-writer). Each file includes YAML frontmatter for Claude Code subagent auto-discovery and defines one agent's purpose, authority, inputs, and outputs. Installed to `.claude/agents/` in the target project
...
7 agent roles form a two-stage pipeline.

**Planning Stage** (`/agent-plan`): **Product** (requirements) → Stage 2a **Architecture** + Stage 2b **UI** (design specs; UI runs only when any task's Needs UI Spec is Yes) → Stage 2c the orchestrator applies the resulting manifest-patch rows itself → Stage 3 **CEO** — one launch, two parts: the risk lenses (security and performance in a single `reviews/risk.md` with its two flag lines; run only when the milestone has a security surface or an applicable performance budget), then the cross-cutting review issuing APPROVED / APPROVED WITH CONDITIONS / REVISION REQUIRED. After an approval verdict, the orchestrator — not Product — backfills the CEO Approval Conditions table.
