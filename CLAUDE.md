...
- **`skills/cast-init/assets/`** — The installable payload. Everything below is relative to this directory:
  ...
  - **`agents/`** — 8 agent role definitions plus a master `README.md` (product, architect, ui, risk, ceo, coder, reviewer, docs-writer). Each file includes YAML frontmatter for Claude Code subagent auto-discovery and defines one agent's purpose, authority, inputs, and outputs. Installed to `.claude/agents/` in the target project
  - **`skills/`** — Pipeline skill definitions (`/agent-plan`, `/agent-code`, `/agent-task`) that orchestrate the planning and engineering stages of the agent workflow plus a mini pipeline for one-off tasks, and the `/cast-doctor` and `/cast-release` maintenance skills (run-anytime install health check plus model-aware documentation audit, and release-prep automation). Installed to `.claude/skills/` in the target project. Each SKILL.md's frontmatter `name` must equal its directory name
  ...
  - **`templates/`** — **Document templates only.** Reusable skeletons (architecture templates, UI spec template, the milestone close record) that agents copy into `artifacts/` as instances. Never filled in place.
  - **`artifacts/`** — **Work artifacts only, grouped by milestone.** Each `milestone-{N}-{slug}/` directory holds one milestone's README (definition), `architecture.md`, `ui.md`, `reviews/` (at most `risk.md`, `ceo.md`, `ux.md`, `risk-impl.md`, `close.md` — the close record instantiates `templates/MILESTONE_CLOSE.md`), per-task files (`tasks/task-{T}-{slug}.md` — one isolated file per task, carrying a Context Manifest and Handoff Log), and per-bug files (`bugs/`). Cross-milestone state (BUGS.md index, STANDUP.md, AGENT_STATE.md) lives at the root; `/agent-task` work goes under `one-off/`. Everything produced by the pipelines lands here.
...
8 agent roles form a two-stage pipeline.

**Planning Stage** (`/agent-plan`): **Product** (requirements) → Stage 2a **Architecture** + Stage 2b **UI** (design specs; UI runs only when any task's Needs UI Spec is Yes) → Stage 2c the orchestrator applies the resulting manifest-patch rows itself → Stage 3 **Risk** (one agent, covering both the security and performance lenses in a single `reviews/risk.md`; runs only when the milestone has a security surface or an applicable performance budget) → Stage 4 **CEO** (final planning-stage review, integrates all prior outputs and issues APPROVED / APPROVED WITH CONDITIONS / REVISION REQUIRED). After an approval verdict, the orchestrator — not Product — backfills the CEO Approval Conditions table.

**Engineering Stage** (`/agent-code`), per task: Step 1 **Coder** (implements, writes and runs tests, commits, and records a verbatim Test Results block in the Handoff Log) → Step 2 **Reviewer** (an independent gate that re-runs verification, classifies findings as Defects or Issues, files bug reports itself, and records an Acceptance Criteria Check). The orchestrator pre-checks that the Test Results block is present before launching Reviewer. Defects that cite a violated acceptance criterion auto-route back to Coder as Fix Now without a Product triage step; other Defects go to Product for triage. Issues route to Coder as a refactor pass, then back to Reviewer. Step 3 is validation: 3a is an orchestrator-only clean close when Reviewer's Acceptance Criteria Check marks every criterion — and, on condition-bearing tasks, every CEO Approval Condition line — Met with evidence; 3b is a Product launch when any criterion or condition line is flagged (`Not met` / `Product judgment`) or a criterion was amended mid-task. When a task's fix resolves a filed bug and the task passes validation, the orchestrator (not Product) flips that bug from Verified to Closed; Product remains the verifier of record for every Approval Condition at the milestone close. At the milestone-completion checkpoint, one Product launch fills `templates/MILESTONE_CLOSE.md` (instanced to `artifacts/milestone-{N}-{slug}/reviews/close.md`) covering per-task validation, milestone-level validation, and the retrospective in a single pass.

**One-Off Task Pipeline** (`/agent-task`): for small self-contained work — bug fixes, typos, single-function refactors, dependency bumps — that does not justify a full milestone plan. Runs the same Coder → Reviewer loop and the same Step 3a/3b validation split as `/agent-code`, but Product validates against the task description rather than a milestone's acceptance criteria. Bails out with a "run `/agent-plan` first" message if the task implies architectural or cross-cutting work.

**Docs Writer** drains the `docs:` queue at task- and milestone-completion checkpoints, but only runs when at least one entry is still pending. The `/cast-release` skill handles release prep and versioning. Process enforcement is the job of the orchestrating pipeline skills themselves; unresolved conflicts escalate to the user. Conflict resolution priority: Product > Architecture > UI.
